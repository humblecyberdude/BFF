...
function Get-Manifest
{
    <#
        ...
        hash value on the client doesn't match what's on the server, the client assumes it needs to be updated and downloads the file via the /common/backstop/v1/files endpoint.
    ...
    ##########################################################################################################################################################################
        ...
        ######################################################################################################################################################################
            ...
            $modules = Get-ChildItem -Path "D:\Apps\Backstop\v1\ClientFiles\modules"
    ...
    $lures = Get-ChildItem -Path "D:\Apps\Backstop\v1\ClientFiles\lures"
...
function Send-File
{
    <#
        .SYNOPSIS
        Used for the endpoint URI https://FQDN/backstop/v[1]/files if the method is a GET. Sends files back
    ...
    ##########################################################################################################################################################################
        ...
        {
            $filePath = "$rootPath\ClientFiles\scripts"
        }

        if($fileExention -eq "psm1")
        {
            $filePath = "$rootPath\ClientFiles\modules"
        }

        if($fileExention -match "ps1xml")
        {
            $filePath = "$rootPath\ClientFiles\etc"
        }

        if($fileExention -match "exe|msi|zip")
        {
            $filePath = "$rootPath\ClientFiles\bin"
        }

        # Verify that the file exists, else return 404. Also, as an added safety, ensure that the filePath never matches anything in the server directory.
        if((Test-Path -Path "$filePath\$fileName")   -and   ($filePath -notmatch "\\v\d{1,3}\\Server"))
        {
...
if(($fileSignatureStatus -eq "Valid")   -and   ($fileSignatureThumbprint -eq "$correctCertThumbprint")    -or   ($correctCertThumbprint -eq "TEST MODE: REPLACE ME"))
            {
...
function Register-Endpoint
{
    ...
    ##########################################################################################################################################################################
        ...
        if($authenticationPassed)
        {
            ...
            {
                # Capture the response for logging variables; send response back to client; log event
                $Global:response = New-PSUApiResponse -StatusCode 400; $response; Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "registerEndpoint" -Comments "Missing headers"; Break
            }
            ...
            {
                $Global:response = New-PSUApiResponse -StatusCode 404; $response; Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "registerEndpoint" -Comments "Client $clientHostname attempted to register but was not found in the ValidHostnames table"; Break
            } 
            ...
            if($clientRegData.clientstate -eq "installed")
            {
                ...
                $Global:response = New-PSUApiResponse -StatusCode 400; $response; Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "registerEndpoint" -Comments "Endpoint already installed"; Break
            }
            ...
            if($clientState -eq "installing")
            {
                ...
                # Capture the response for logging variables; send response back to client; log event
                $Global:response = New-PSUApiResponse -StatusCode 200 -Body $jsonResponse; $response; Write-EndpointLog -LogType Access -severityLevel "info" -eventTag "registerEndpoint" -Comments "Gave client the random directory and task path names"; Break
            }
            ...
            if($clientState -eq "installed")
            {
                ...
                # Capture the response for logging variables; send response back to client; log event
                $Global:response = New-PSUApiResponse -StatusCode 200; $response; Write-EndpointLog -LogType Access -severityLevel "info" -eventTag "registerEndpoint" -Comments "Client should now be installed"; Break
            }
...
function Remove-Endpoint
{
    ...
    ##########################################################################################################################################################################
        ...
        if($authenticationPassed)
        {
            ...
            {
                # Capture the response for logging variables; send response back to client; log event
                $Global:response = New-PSUApiResponse -StatusCode 400; $response; Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "removeEndpoint" -Comments "Client request missing headers"; Break
            }
            ...
            {
                New-PSUApiResponse -StatusCode 404; Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "removeEndpoint" -Comments "An uninstall attempt was made for a client not found in the database"; Break
            }
            ...
            if(($clientState -eq "uninstalling")   -and   ($reason -match "EXAMPLE"))
            {
                if($clientRegData.clientState -eq "installed")
                {
                    ...
                    # Capture the response for logging variables; send response back to client; log event
                    $Global:response = New-PSUApiResponse -StatusCode 200 -Body $jsonResponse; $response; Write-EndpointLog -LogType Access -severityLevel "info" -eventTag "removeEndpoint" -Comments "Gave client back the directory and task names for removal"; Break

                } Else {

                    $Global:response = New-PSUApiResponse -StatusCode 400 -Body "Bad request"; $response; Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "removeEndpoint" -Comments "An uninstall attempt was made for this client but Backstop DB didn't show this client had Backstop installed at all"; Break
                }
            }
            ...
            if(($clientState -eq "uninstalled")   -and   ($reason -match "EXAMPLE"))
            {
                ...
                Add-Content -Value "$apiKey" -Path "D:\Apps\Backstop\$environment\db\revocationList.txt"
            }
            ...
            if($reason -notmatch "EXAMPLE")
            {
                ...
                # Capture the response for logging variables; send response back to client; log event
                $Global:response = New-PSUApiResponse -StatusCode 200 -Body $jsonResponse; $response; Write-EndpointLog -LogType Access -severityLevel "warn" -eventTag "removeEndpoint" -Comments "Fake Uninstall"; Break
            }
        }
...
function Send-SplunkHEC
{
    ...
    ##########################################################################################################################################################################
        ...
        if($authenticationPassed)
        {
            ...
            {
                $Global:response = New-PSUApiResponse -StatusCode 400 -Body "JSON body was invalid"; $response; Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "relay" -Comments "Client tried to relay a message to Splunk HEC but the JSON body was invalid"; Break
            }
            ...
            Write-EndpointLog -LogType Access -severityLevel "info" -eventTag "relay" -Comments "Message relayed to Splunk"; break
        }
    ...
    ##########################################################################################################################################################################
        ...
        if($authenticationPassed)
        {
            ...
            if($updateType -eq 'database')
            {
                ...
                [System.IO.File]::WriteAllBytes("D:\Apps\Backstop\$environment\tmp\$fileName", $Data)

                # Get hash and write to the log file
                $uploadedFileSHA256 = (Get-FileHash -Path "D:\Apps\Backstop\$environment\tmp\$fileName" -Algorithm SHA256).hash
                Write-EndpointLog -LogType Access -Comments -severityLevel "info" -eventTag "update" "Processed file upload $fileName with a SHA256 hash of $uploadedFileSHA256 from $clientHostname"

                # Ensure that the file is correctly signed and valid (and signed only by us)
                if((Get-AuthenticodeSignature -FilePath "D:\Apps\Backstop\$environment\tmp\$fileName").SignerCertificate.Thumbprint -eq $correctBackstopLowTrustCertThumbprint)
                {
                    # Remove older files
                    Remove-Item -Path "D:\Apps\Backstop\$environment\tmp\validEndpointList.txt" -Force -ErrorAction SilentlyContinue
                    Remove-Item -Path "D:\Apps\Backstop\$environment\tmp\hashedUserData.csv" -Force -ErrorAction SilentlyContinue
                    ...
                    #>
                    & "C:\Program Files\7-Zip\7z.exe" x "D:\Apps\Backstop\$environment\tmp\$fileName" -o"D:\Apps\Backstop\$environment\tmp" -bso0 -y
                    ...
                    #region   UPDATE VALID ENDPOINTS LIST IN DB   ############################################################################################################
                        ...
                        if(Test-Path -Path "D:\Apps\Backstop\$environment\tmp\validEndpointList.txt")
                        {
                            # Read the data into a variable
                            $validEndpoints = Get-Content -Path "D:\Apps\Backstop\$environment\tmp\validEndpointList.txt"
                            ...
                            try {
                                ...
                                Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "update" -Comments "Database load error! Should add more details here... Error: $_" 
                    ...
                    # Update process for valid endpoint list (valid hostnames that are able to be registered)
                    if(Test-Path -Path "D:\Apps\Backstop\$environment\tmp\hashedUserData.csv")
                    {
                        # Connect to the SQLite Database
                        $dbPath = "D:\Apps\Backstop\$environment\db\backstop.db"
                    ...
                    Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "update" -Comments "bad sig" 
                }
            }
            ...
            #region   FILE UPDATE PROCESS   ##################################################################################################################################
                ...
                {
                    $Global:response = New-PSUApiResponse -StatusCode 501 -Body "Not implemented yet"; $response; Write-EndpointLog -LogType Access -severityLevel "error" -eventTag "update" -Comments "Client tried to update files but that function hasn't been implemented yet"; Break
                }
...
function Get-Tokens
{
    <#
        ...
        .EXAMPLE
        Invoke-RestMethod -Method GET -Uri "https://FQDN/backstop/common/backstop/v1/tokens" -Headers @{apiKey = "$apiKey"; clientHostname = $env:COMPUTERNAME; kind = "aws-id"; memo = "Host=$env:COMPUTERNAME User=$localUser DIR=YOUR_DIR_HERE NOTES=Your Notes Here"} -UserAgent "Backstop"
    ...
    ##########################################################################################################################################################################
        ...
        if($authenticationPassed)
        {
            if($kind -eq "aws-id")
            {
                ...
                # Capture the response for logging variables; send response back to client; log event
                $Global:response = New-PSUApiResponse -StatusCode 200 -Body $jsonResponse; $response; Write-EndpointLog -LogType Access -severityLevel "info" -eventTag "getCanaryTokens" -Comments "Gave client the $kind token"; Break
    ...
    ######################################################################################################################################
        ...
        if($authenticationPassed)
        {
            ...
            if($personaUserDetails)
            {
                ...
                $Global:response = New-PSUApiResponse -StatusCode 200 -Body $personaUserDetails; $response; Write-EndpointLog -LogType Access -severityLevel "info" -eventTag "getPersona" -Comments "Gave client the persona info"; Break
                ...
                # Give persona details back to the client
                $Global:response = New-PSUApiResponse -StatusCode 200 -Body "$personaUserDetails"; $response; Write-EndpointLog -LogType Access -severityLevel "info" -eventTag "getPersona" -Comments "Unable to find persona details based on hashed username supplied. Hash supplied was $hashedUsername"; Break
            }
        }
